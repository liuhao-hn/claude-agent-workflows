...
\subsection{获取 Skill 包}

你会拿到一个或多个 \texttt{.tar.gz} 文件，放到桌面上。

两个包可选：

\begin{itemize}[nosep]
    \item \texttt{claude-skills-all.tar.gz}（推荐）：全部 22 个 Skill，含自定义 + Minimax + Stata
    \item \texttt{claude-skills-export.tar.gz}：7 个自定义 + Stata Skill
\end{itemize}

\subsection{解压安装}

\textbf{Mac 用户——打开终端，输入：}

\begin{shcode}
# 安装全部 22 个 Skill（推荐）
tar -xzf ~/Desktop/claude-skills-all.tar.gz -C ~/.claude/skills/

# 或仅安装 7 个 Skill（自定义 + Stata）
tar -xzf ~/Desktop/claude-skills-export.tar.gz -C ~/.claude/skills/
\end{shcode}

\textbf{Windows 用户——打开 PowerShell，输入：}

\begin{shcode}
# 安装全部 22 个 Skill（推荐）
tar -xzf "$env:USERPROFILE\Desktop\claude-skills-all.tar.gz" -C "$env:USERPROFILE\.claude\skills\"

# 或仅安装 7 个 Skill（自定义 + Stata）
tar -xzf "$env:USERPROFILE\Desktop\claude-skills-export.tar.gz" -C "$env:USERPROFILE\.claude\skills\"
\end{shcode}

如果 Windows 提示找不到 tar 命令，先安装 7-Zip 手动解压，再把解压出的文件夹复制到 \texttt{C:\textbackslash Users\textbackslash 你的用户名\textbackslash .claude\textbackslash skills\textbackslash} 目录下。
...
\subsection{本 Skill 包包含的能力}

\subsubsection*{自定义 Skill（6 个）}
...
\midrule
{\small\texttt{academic-search}} & 经济学论文搜索、引用分析，支持中英文多平台（arXiv、SSRN、NBER、知网等） \\
\midrule
{\small\texttt{coder-critic-review-team}} & 多 Agent 代码审查协作工作流 \\
\midrule
{\small\texttt{deeppapernote}} & 论文深度阅读笔记生成（输出 Obsidian 格式） \\
\midrule
{\small\texttt{gaodun-essay-grader}} & 申论大作文五维度批改（40分制评分体系） \\
\midrule
{\small\texttt{md2pdf}} & MD→TEX→PDF 专业中文排版（ctexart + booktabs 风格） \\
\midrule
{\small\texttt{workflow-to-skill}} & 工作流自动沉淀引擎（任务结束 → 生成 Skill → 更新 Memory） \\
\bottomrule
\end{tabular}
\par}
...
\subsubsection*{Minimax Skill（14 个）}
...
\textbf{Skill} & \textbf{说明} \\
\midrule
\texttt{minimax-docx} & 专业 DOCX 文档创建、编辑和格式化 \\
\midrule
\texttt{minimax-pdf} & 高质量 PDF 生成（创建/填充/重排） \\
\midrule
\texttt{minimax-xlsx} & Excel 电子表格创建、分析和格式化 \\
\midrule
\texttt{pptx-generator} & PowerPoint 演示文稿生成和编辑 \\
\midrule
\texttt{android-native-dev} & Android 原生开发指南（Material Design 3/Kotlin） \\
\midrule
\texttt{flutter-dev} & Flutter 跨平台开发（Riverpod/GoRouter） \\
\midrule
\texttt{ios-application-dev} & iOS 应用开发指南（UIKit/SnapKit/SwiftUI） \\
\midrule
\texttt{react-native-dev} & React Native/Expo 开发指南 \\
\midrule
\texttt{frontend-dev} & 前端全栈开发（动画/媒体/文案） \\
\midrule
\texttt{fullstack-dev} & 全栈后端架构与前后端集成 \\
\midrule
\texttt{minimax-multimodal-toolkit} & MiniMax 多模态模型（语音/音乐/视频/图像生成） \\
\midrule
\texttt{gif-sticker-maker} & 照片转 GIF 动画贴纸 \\
\midrule
\texttt{shader-dev} & GLSL 着色器特效开发 \\
\midrule
\texttt{vision-analysis} & 图像分析与描述（OCR/图表/截图理解） \\
\bottomrule
\end{tabular}
\par}
...
\textbf{Skill} & \textbf{说明} \\
\midrule
\texttt{stata} & Stata 代码完整参考（.do 文件、计量经济学、因果推断、Mata 编程） \\
\midrule
\texttt{stata-c-plugins} & Stata C 插件开发指南 \\
\bottomrule
\end{tabular}
\par}
...
\textbf{汇总：}共 4 个 Marketplace，提供 22 个可打包分享的 Skill + 36 个需在线安装的代码插件。
...
\midrule
$\square$ & Skill 包已解压到 \texttt{\textasciitilde/.claude/skills/} 目录 \\
\bottomrule
\end{tabular}
\par}
