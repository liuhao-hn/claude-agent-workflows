...
description: |
  ...
  核心协议借鉴 changocr/agents-lab：TASKS.md 任务黑板 + artifacts/ 持久消息总线 + creator/reviewer/verifier 职责分离 + 契约化派发。
---
...
| 角色 | 工具 | 职责 | 权限 |
|---|---|---|---|
| 指挥官 | Claude Code | 制定规则、拆解任务、写契约、按名册分配、只读审查、验证证据、更新黑板、总结进度与规划报告 | 可读写 TASKS.md 与 artifacts/ |
| 执行者（名册） | zcode / codex / Claude子代理 | 按契约实现、产出代码与证据 | 只读写契约指定范围 |
...
|---|---|---|---|---|
| `example-task` | Claude/Codex | BACKLOG | `artifacts/contract/xxx.md` | None |
...
## 任务契约模板（唯一权威，派发前必须自包含）

契约本质是**规则集**，执行者必须逐条遵守，任何一条未满足即未完成。执行者会话看不到指挥官上下文，契约必须能独立成立。`caw new-task` 以 `templates/contract.md.tpl` 为唯一模板生成，此处与之保持一致：

```markdown
# 任务：{任务名}

> 本契约是任务规则集，执行者必须逐条遵守；任何一条未满足，任务即未完成。
...
2. **写契约**：每个待派发任务写 `artifacts/contract/{task}.md`，正文全部进文件，聊天只传状态与路径。
3. **派发**：按「执行者名册与路由」选定执行者：
   - `zcode "读 artifacts/contract/{task}.md 并严格执行"`
   - `codex "读 artifacts/contract/{task}.md 并严格执行"`（或 `codex-deepseek`）
   - 只读类工作派给 Claude 子代理
4. **执行**：执行者实现并汇报证据（状态 + 路径 + 命令输出）。**全程不声明未运行过的验证。**
5. **审查**：Claude 只读审查 diff 与契约，按 `CRITICAL/HIGH/MEDIUM/LOW` 分级；`CRITICAL`/`HIGH` 阻断完成，退回执行者修订后复审。报告原文存 `artifacts/review/`。
...
- **最小改动**：满足契约即可，不顺手改相邻代码或格式。
- **不越权**：缺授权、涉隐私数据、未解决 CRITICAL/HIGH 时停下并上报指挥官。
- **上下文交接代价**：每次 Claude↔各执行者切换以契约文件为准，避免口头传上下文；不同执行者互不相通，契约要更自包含。

## 应用示例

- 长项目初始化：指挥官拆 5 个任务 → 写 5 份契约 → 逐个派给执行者 → 审查+验证后更新 TASKS.md → 下班写 handoff。
- 多执行者协作：zcode 跑全量数据/批处理，codex 修疑难 bug，Claude 子代理做只读审查；Claude 汇总各执行者证据，更新 TASKS.md 并输出阶段进度与规划报告。
- 疑难 bug：把"复现+定位+修复+单测"打包成一份契约丢给 GPT，Claude 只审 diff 与测试证据，省 DeepSeek 上下文。
